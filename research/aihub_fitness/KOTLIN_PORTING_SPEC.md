...
- 컬 신호는 `elbow_mean` 유지. 후보 `elbow_minside` 는 양팔이 보일 때만 쓰는 형태 — 앱 피처의 한쪽 폴백은 교대 컬 가림에서 과다 카운트, 팔별 카운터는 동시 컬 가림에서 두 번 센다. MM-Fit 영상(MediaPipe) 뒤 결정.
...
- 상수의 출처: 게이트·f·불응기·maxGap 은 설계값(데이터로 조정하지 않음). 평활 없음과 첫 쌍 8 s 는 MM-Fit(개발 데이터)에서 골랐다 — 8 s 는 결과 전에 적은 규칙으로.
  배터리 결과의 §12 해시는 결과와 규칙 본문을 묶을 뿐 작성 순서의 증거가 아니다 — 커밋할 때 §12 만 담은 커밋을 먼저 남긴다.
