...
렙 라벨 없는 AIHub 에서 렙 신호를 고르는 원리: **클립별 합의(consensus)** — 몸 전체가 렙 주기로 움직이므로 (종목×피처) 전수에 자기보정 카운터를 돌려 다수 피처가 합의하는 카운트를 참값 프록시로 삼고, 합의 일치율이 최대인 피처를 채택. 카운터는 스쿼트 육안 정답 6클립으로 보정(성긴 샘플에서 p40/p60 좁은 밴드, 오차 8→3).

- **서서 하는 32종목: 전부 판별 성공** (합의일치 0.77~0.95). 스쿼트→`knee_minside` 0.94, 데드→`shoulder_neck_gap` 0.95, 스티프→`sh_over_hip_fwd` 0.95, 런지류→`stance_w`/`hip_minside` 0.92~0.95, 컬→`elbow`/`palm_fwd` 0.84~0.85, OHP·랫풀·딥스·풀업→팔꿈치 계열 0.77~0.88, 레터럴→`grip_w` 0.88 — 운동학과 전부 정합.
- **바닥 9종목: AIHub 0.6s 주석으로 판별 불가** (전 피처 0.65~0.81 동률, 중앙 2회 언더카운트) → 기기 실측이 결정: 푸시업 `wrist_shoulder_d` 가 3.3fps 에서 정답 3~4회를 4회로 적중. 크런치 `head_ground`·레그레이즈 `hip_ang`·힙쓰 `hip_dev_ankle` 등은 운동학 채택 후 M0 검증.
- **플랭크 = 등척성 확정**: 설문의 '2회'는 진입+이탈 아티팩트 — 상태기계 SETTLING/END 가 흡수하면 ACTIVE 카운트 0 = HoldTimer 로 자연 위임.
- **교대형**(시저·바이시클·니업·사이드크런치): 좌우 역위상이 0.6s 에일리어싱으로 검출 불가 — 정의 기반 분류, 기기 로그 재검.

**빠른 렙 정책 (실측 근거)**: 렙 주기 중앙값 3.2s(서서, 렙당 0.6s 기준 5.3샘플) — 절반 서브샘플 시 카운트 회복률 **0.50 으로 붕괴** → **최소 렙당 ~4샘플** 하한. 앱 3.3fps 는 주기 ≥1.2s 까지 안전(실측 주기 2.4~4.8s 라 평상시 여유 2~4×). 대처 사다리: ① 주기추정 기반 ACTIVE 한정 5~6fps 부스트(0.7s 주기까지) ② 측정주기 ≈ 2~3×샘플간격이면 에일리어싱 자가진단 → 카운트 미확정 표시 ③ 반사이클 카운트 폴백 ④ UX 최후선("조금 천천히" — 통제된 템포는 폼 코칭과도 정합). 카운터 구현 규약: **평활 창과 밴드 폭 모두 샘플 밀도에 적응**(성기면 원시값+p40/p60, 촘촘하면 3점중앙값+p30/p70) — 성긴 신호에 평활을 쓰면 렙 꼭대기가 지워진다(1차 실행 실패 원인).
