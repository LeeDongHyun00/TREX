...
## §36 — 바닥 실시간 음성·부위 피드백 UX (2026-09-11)

사용자는 §35의 참고 기록만으로는 운동 중에 문제를 알아차릴 수 없으며, 음성과 빨간 부위 표시가 필요하다고 요청했다. 이 명시적 요청으로 바닥 실시간 참고 피드백의 일괄 음소거 정책을 변경한다. 기존 AGENTS/§31의 beta 침묵 규정은 이 경로에 대해 이전 결정이다. 규칙 등급·임계값·점수 정책·서서 종목의 beta 음소거는 바꾸지 않는다. 교차검증/실기기 정확도가 확보됐다는 뜻이 아니다.

### 동작

`FloorFeedbackController`가 준비, 측정, 확인 필요, 복귀, 관측 불가, 일시정지를 관리한다. 같은 출력 객체의 문구·landmarks·speech를 라이브 패널, 골격, TTS에 연결했다.

- 플랭크는 초기 기준 대비 같은 방향 이탈 2초를 만족하면 골반/몸통을 붉게 표시하고 방향을 말한다. 첫 이탈 기록과 현재 상태를 분리했다. 과거에 한 번 이탈했어도 복귀한 현재 자세가 계속 붉게 남지 않는다.
- 푸시업·니푸쉬업·힙쓰러스트는 두 번 연속 범위 미달이 검출될 때 안내한다. 같은 RepRecord 중복, 가림/중단 전후, 15초 이상 떨어진 반복을 연속 미달로 합치지 않는다. 경고는 최대 5초 유지하고 다음 유효 반복이면 빨강을 지운다. 두 번 연속 범위 충족과 1초 확인 후에 복귀 안내한다.
- 나머지 활성 window 규칙은 최근 창 위반이 1.5초 더 지속되어야 안내한다. 현재 프레임에 해당 피처가 없으면 후보에서 제외한다. 머리 규칙은 목/머리 쪽을 강조하며 피처 계산의 기준점인 골반을 오류 부위로 표시하지 않는다.
- 음성은 경고 앞에 '참고 안내예요'를 붙이고 같은 항목 12초, 전체 4초 간격으로 제한한다. 음소거/음성 엔진 준비 전에는 경고 발화 쿨다운을 소모하지 않는다. 초기 준비·측정 시작·실제 범위 복귀도 알린다. 참조값에서 벗어났다는 관측을 전달하며 정답 자세나 안전을 인증하지 않는다.
- 유보·결측·시간 만료는 복귀가 아니다. 관측이 끊기면 빨강과 연속 증거를 지운다. 기록 모드에서는 모집단 기준 window/rep 경고를 억제하고 플랭크의 본인 기준 변화만 전달한다.
- 크런치·Y 레이즈는 동작 측정 시작·촬영 안내를 말하고 못 보는 부위를 밝힌다. 제외한 견갑골/목 규칙으로 빨간 오류를 만들지 않는다.

### 화면·음성 배선

- 바닥 골격의 가시성 기준을 피처와 같은 0.35로 맞췄다(종전 0.5 때문에 측정되지만 빨강이 사라질 수 있었음). 주목 부위에는 굵은 연결선과 원형 강조를 사용한다.
- `RuleHighlight`는 가장 긴 접두사를 선택해 `wrist`가 `wrist_shoulder_d`의 어깨 표시를 가리던 문제를 수정했다.
- 주 문구의 검증 없는 '좋아요, 자세 유지'를 바닥에서 제거했다. '확인 필요 · 참고' 등의 상태와 한 가지 안내를 우선 표시하고 촬영/수치 상세는 접어둔다. 바닥의 의미 없는 점수 대시는 '피드백 · 참고'로 교체했다.
- 음성 켜짐/꺼짐, 접근성 동작명, '음성 확인' 재생 버튼을 추가했다. 일시정지·음소거는 대기 음성을 중단한다. 실제 합성 오류도 화면에 노출한다.
- Android 11 이상 음성 엔진 검색에 필요한 TTS_SERVICE queries를 manifest에 추가했다. 근거: https://developer.android.com/reference/android/speech/tts/TextToSpeech .

### 검증과 한계

- 전체 JVM 테스트 168건(추가 `FloorFeedbackTest` 15건) 통과. 준비/지속/복귀/가림/음소거/쿨다운/중복 렙/제외 규칙/기록 모드/부위 매핑을 검사한다. APK 빌드 통과.
- 연결된 SM-N976N에 데이터 삭제 없이 덮어쓰기 설치. `FloorFeedbackDeviceTest`에서 실제 한국어 TTS 초기화와 재생 완료 콜백 통과(1건). 스피커로 실제 들리는 음량·이어폰 연결·실제 운동 자세 정확도·카메라 화면에서의 골격 위치는 이 테스트가 보증하지 않는다.
- Gradle connectedDebugAndroidTest는 오프라인 UTP 캐시 누락으로 실행 전 실패했다. 이미 빌드한 앱/테스트 APK를 adb로 설치한 뒤 AndroidJUnitRunner를 직접 실행해 기기 테스트를 완료했다. 기기 테스트가 실패했다고 숨긴 것이 아니며 실행 경로를 구분한다.


## §37 폴더블 대응·편집 동작·말투·데모 데이터 정리 (2026-09-12)

사용자 요청: 폴더블 실기기가 없어도 구현, 운동 카드 짧은 탭으로 수정, 로그인 로고 중앙 정렬, 모든 설명을 `~룡` 말투로 통일, 칼로리·출석 등 샘플 데이터 제거. 기존 §36 바닥 피드백 작업은 유지한다.

### 창 크기·접힘·상태 유지

- `AdaptivePolicy.kt`는 현재 레이아웃 제약의 폭/높이와 실제 힌지 사각형을 입력받는다. 가로·세로 및 두께 0인 접힘을 처리한다. WindowManager 1.5.0의 `WindowInfoTracker`에서 분리 또는 완전 가림에 해당하는 `FoldingFeature`만 사용한다. 창 좌표를 각 콘텐츠의 `positionInWindow` 기준으로 변환하여 시스템 바 여백·부분 창과 좌표계가 섞이지 않게 했다.
- 일반 화면은 힌지를 피한 더 큰 영역 하나에 두고 최대 폭 840dp, 로그인/복구/온보딩·시트는 600dp로 제한한다. 자세 화면은 힌지가 있으면 그 양쪽, 없으면 폭 600dp 이상인 가로 창 또는 840dp 이상에서 좌우, 그 외에는 위아래로 배치한다. 이 값은 앱 배치 정책이며 기종 판별값이 아니다.
- 카메라와 패널은 `Layout`의 고정된 두 자식으로 남는다. 크기 전환 때 측정·위치만 바꾸므로 `AndroidView`의 같은 PreviewView를 여러 부모에 연결하거나 코치/집계기/앵커를 교체하지 않는다. `FIT_CENTER`, 원본 분석 이미지, 판정·규칙 임계값·모드 의미는 유지한다.
- `DisplayManager`로 실제 디스플레이 회전을 관측하고 Preview와 ImageAnalysis의 targetRotation을 함께 갱신한다. manifest의 기존 configChanges에 density/fontScale을 추가해 접힘·글씨 변경에 따른 세션 재생성을 막는다. 프로세스 종료 후 세트 내부 상태 복원은 이 구현의 보장 범위가 아니다.
- 루트 safeDrawing 인셋, 로그인/계정 복구/가이드/시트 스크롤, 낮은 창의 간결한 타이머, 최소 높이 기반 CTA를 적용했다. 자세 설명은 스크롤하고 일시정지/완료는 조작 영역에 고정한다. 좁은 폭에서는 FlowRow로 줄을 바꾼다. 하단 메뉴와 기록 헤더의 실제 높이를 측정해 콘텐츠 여백에 반영한다. 횟수 스테퍼는 숫자와 단위를 한 줄에 표시한다.

### 운동 카드·로고·설명

- 운동 카드 짧은 탭은 `MainSheet.Sets`를 연다. 카드별 연필/세트 수정 버튼은 제거했다. 화살표는 상세 펼침, 길게 누르기는 순서 변경 역할을 유지한다.
- 로그인 PNG는 720×1280 투명 캔버스의 위쪽에 로고가 있었다. 정사각형 카드를 ContentScale.Crop으로 채우면 로고가 잘린다. 16프레임 공통 원본 영역 `(180,180,360,360)`만 그려 카드 중앙에 맞췄다. 프레임마다 경계가 달라져 애니메이션 중심이 흔들리지 않게 같은 영역을 사용한다. 원본 이미지는 변경하지 않는다.
- 사용자의 새 요청이 AGENTS의 이전 존댓말 문체보다 우선한다. `TrexText`/`toDinoCopy`가 화면 출력과 TTS 합성 직전에 문장 어미를 바꾸고, 가이드·온보딩 설명은 직접 다듬었다. 입력값·운동/음식 이름·수치·단위는 보존한다. 판정 원문·로그·규칙 키는 바꾸지 않는다. '참고', '검증 중', '판정 없음'의 의미와 음성 발화 조건도 그대로다.
- 완료 제목에서 판정 없이 '정확하게', 비교 데이터 없이 '좋아지고 있다'고 말하던 표현을 실제 완료·기록 안내로 바꿨다. 사진 식단 분석의 고정 결과와 신뢰도는 제거하고 준비 중임을 알린 뒤 수동 기록으로 연결한다.

### 데이터

- 새 사용자의 운동 기록·출석·먹은 음식은 빈 상태에서 시작한다. 음식 카탈로그와 추천 운동 템플릿은 기록 데이터가 아니므로 유지한다. 가짜 날씨·추천 메뉴 칼로리·가짜 평균 비교는 제거했다. 기록 없는 날짜를 실제 휴식으로 단정하지 않는다.
- `LegacyDemoData`는 구버전 생성기의 완전한 지문만 보관한다. 현재 추천 루틴/계산 함수가 바뀌어도 지문은 바뀌지 않는다. 저장소 초기화 때 전체 항목과 구버전 평균값이 일치하는 기록, 전체 음식 슬롯이 일치하는 식단만 제거한다. 일부 값이 일치하거나 수정된 기록은 보존한다.
- 제거 전에 `before_demo_cleanup_history`/`before_demo_cleanup_diet`에 원본 JSON을 동기 저장한다. 백업 실패 시 정리하지 않는다. `demo_cleanup_v1`으로 1회 수행하며 실제 기록과 백업은 지우지 않는다.
- 운동 중 다음 버튼을 누를 때 해당 운동의 일시정지 제외 진행 시간을 초로 기록한다. 미완료 운동은 기록하지 않는다. `workoutId`와 `durationSeconds`는 저장·복원되며 구버전은 분 단위 값으로 읽는다. 여러 1분 미만 운동은 초를 합한 뒤 분으로 변환한다. 이어하기 시 기존 완료 운동의 시간을 복원한다.
- 소모 칼로리는 이 시간과 종목별 계수를 곱한 **추정값**이다. 센서 측정값이나 개인별 정밀 계산이 아니다. 0초는 0kcal이고, 계획 시간 또는 최소 칼로리를 실제 소모처럼 채우지 않는다. 음식 영양 기준값·권장 목표 계산은 유지한다.

### 검증

- `gradlew.bat :app:testDebugUnitTest :app:assembleDebug :app:assembleDebugAndroidTest` 통과. JVM 176건(새 AdaptiveAndDataTest 8건 포함).
- 전용 API 36.1 에뮬레이터에 앱/테스트 APK를 설치하고 AndroidJUnitRunner로 `AdaptiveUiTest` 9건 통과. 크기 변경과 합성 힌지에서 카메라 Composition 유지, 짧은 탭/긴 누르기/상세 화살표, 실제 편집 팝업과 저장, 로그인 값 유지·로고, 글씨 2배·낮은 창의 타이머/가이드, 실제 자세 화면의 일시정지/완료, 빈 저장소·초 단위 왕복·데모 백업/실제 기록 보존을 검사했다.
- 테스트 좌표 범위: JVM 폭 320/359/599/600/839/840/1200, 높이 320/479/480/900; UI 주요 창 360×360, 360×640, 360×720, 600×360, 600×800, 840×700/720dp. UI 캡처는 `app/build/layout-qa/`에 있고 육안으로 로고·편집·타이머·자세 패널을 확인했다.
- 실제 폴더블은 연결되지 않았다. 실제 접힘 이벤트·제조사별 카메라 재연결·분석/오버레이 정합·태스크바/멀티윈도우·스피커 발음은 기기에서 추가 확인해야 한다. 에뮬레이터 통과를 자세 판정 정확도 검증으로 해석하지 않는다.
