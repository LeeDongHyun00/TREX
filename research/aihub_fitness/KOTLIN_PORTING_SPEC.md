...
AIHub 조건 116개를 해부학적 자유도로 분류해, 완벽한 GT 3D 에서의 상한과 MediaPipe 전이 후를 비교했다. **답: 모든 자세를 파악할 수는 없다.**
...
| **척추 곡률** | 4 | 0.674 | 0.677 | **0%** | ❌ 원리적 불가 |

**실패 유형 2가지**: ① *원리적 불가*(척추 곡률·긴장/부하) — 완벽한 좌표로도 안 잡힘, 정의가 좌표에 없는 것을 가리킴. ② *MP 붕괴*(축회전·견갑/골반) — GT 에선 그럴듯하나 랜드마크 정밀도로 무너짐.

**뷰 의존**: 무릎 내/외(valgus)는 **정면 필수** — 스쿼트 MP AUC 정면 0.993 vs 전방사선 0.79~0.82, 데드리프트 0.909 vs 0.51~0.67(피처 충실도 r: 정면 0.64 → 후방사선 0.26). 사선 뷰에서는 이 규칙을 **유보(ABSTAIN)** 하는 편이 오탐보다 낫다.

**설계 함의**: 앱이 판정할 수 있는 것은 **관절이 얼마나 굽었나 · 사지가 어디에 있나 · 몸통이 어디를 향하나 · 얼마나 흔들리나** 넷뿐이다. "힘주세요/견갑 고정/척추 중립" 같은 지시는 검증 불가 — 코칭 문구로 쓰더라도 판정 근거는 관측 가능한 프록시임을 명시할 것. 축회전이 핵심인 종목은 **회전 대신 그 결과인 위치 변화로 조건을 다시 정의**해야 한다(§21 요건 3).
