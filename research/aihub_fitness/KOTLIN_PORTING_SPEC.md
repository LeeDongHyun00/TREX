...
## §56 — 관측 경계·복귀 카운트·전달 후 회복 계약 초안 (2026-09-17)

**검증: 2026-09-17 JVM 303건 통과(실패·오류·생략 0), `:app:assembleDebug` 성공.** 연속 복귀·짧은 가림·관측 경계·뷰 유보·전달 후 관측 회복의 코드 회귀 검증이다. 실제 기기 촬영 정확도·교정 효과·배포 완료를 뜻하지 않는다. 이번 세션에 연결된 Android 기기가 없으며 이전 §55의 Note10+ 결과를 이번 변경의 증거로 재사용하지 않는다.

### 56.1 배경과 범위

사용자는 26개 운동의 실제 횟수·교정 엔진과 함께, 일부러 한 발 들기·무릎을 바깥으로 보내기·발을 옆으로 옮기기 등 세부 편차까지 구체적으로 다루도록 요청했다. 세부 문구를 추가하기 전에 관측이 끊기거나 촬영 기준이 바뀐 데이터를 연결해 횟수·교정 성공을 만드는 경로를 차단해야 한다. 이 변경은 공통 관측·반복·음성 계약을 보강한다. 모든 세부 이벤트 검출기를 앱에 탑재한 변경은 아니다.

직접 변경을 확인한 주요 파일은 `ObservationEpoch.kt`, `PostureLive.kt`, `PostureAnalyzer.kt`, `RepCounter.kt`, `ReturnRepTracker.kt`, `PostureCoach.kt`, `PostureRules.kt`, `PostureAssessment.kt`, `PostureSetReport.kt`다. 기존 ship/beta/exclude의 의미를 유지하고, 미검증 편차를 기존 ship 규칙의 자격으로 승격하지 않는다.

### 56.2 관측 epoch와 단조 시계

- `ObservationEpoch`는 추론 시작 시 ticket을 발급하고, 결과 반영 직전에 현재 ticket인지 확인한다. 경계 초기화와 결과 반영은 같은 락으로 직렬화하며 추론 자체는 락 밖에서 실행한다. 번호는 정상 자세의 인증값이 아니다.
- 운동/규칙셋 초기화, 앞·뒤 카메라 전환, 화면 회전, 준비/일시정지 상태 전환이 관측 경계를 만든다. 이전 카메라에 결합된 analyzer도 현재 카메라 선택과 다르면 프레임을 반환한다. 전환 전에 시작한 추론이 새 집계 창·카운터·로그에 들어가지 않도록 한다.
- 실시간 추론·반복·코칭 전달 시각은 `SystemClock.elapsedRealtime()`의 같은 단조 ms를 쓴다. 중복·역행 시각은 버리고 진행 상태를 무효화한다. 세트 로그는 같은 단조 기준에서 계산한 상대시각을 보존한다. 벽시계 변경을 운동 속도나 전달 지연으로 읽지 않는다.
- 카메라/회전 변경은 현재 코칭·강조·앵커·바닥 피처·개인 비교를 새로 시작하게 하며 완료한 반복 수를 초기화하지 않는다. 일시정지는 진행 반복과 현재 관측 창·전달 확인을 비우고 준비/세션 정책에 맞춰 재개한다.

### 56.3 짧은 가림도 누락된 반복을 이어 붙이지 않음

`RepCounter.onObservationLost()`는 출발 기준, 진행 반복, 평활/주기 추정을 폐기하고 완료한 수·시각·마지막 완료 극값을 보존한다. `null`, 비유한 값, 물리 범위 밖 신호는 짧은 한 표본이라도 손실이다. 분석이 아예 생략되거나 사람이 없어진 호출부도 이 API를 호출한다. 유효 표본 간 긴 공백의 기본 경계는 1,500ms이며, 이 숫자는 현재 검출 정책이지 체형·환경별 정확도를 보장하는 보정값이 아니다.

미검출·일시정지·분석 예외의 시각과 빈 피처를 원본 관측 로그에 남긴다. 최종 평가 재생에서도 그 마커를 소비하므로 300ms짜리 한 프레임 가림을 600ms의 정상 유지 시간으로 연결하지 않는다. 세트 마감은 결과 반영과 같은 epoch 락에서 샘플·반복·경계를 일관되게 복사한다.

실시간 코치도 실제 운동 피처가 없는 프레임, 중복·역행 시각, 긴 공백에서 현재 창과 위반 연속성·전달 확인을 비운다. 방향 피처만 존재하는 프레임으로 앵커 폴백의 관측 시간을 채우지 않는다. 가림 후 사용자가 이미 복귀했더라도 보지 못한 운동을 보간하여 한 번으로 세지 않는다.

`PostureAnalyzer`는 화면 밖 좌표·비유한 world 좌표와 visibility/presence 부재를 신뢰도 높은 관절로 승격하지 않는다. world 배열 길이도 확인한다. 이것이 발 접촉이나 관절의 의학적 정확성을 인증하는 것은 아니다.

### 56.4 정지 없는 연속 복귀 FSM

실시간 `completeOnReturn` 경로의 단계는 `PREPARING → READY → OUTBOUND → RETURNING → READY`다. 관측된 출발 구간에서 충분히 이탈한 뒤 방향을 바꾸어 출발 구간으로 돌아오면 그 복귀 프레임에서 완료한다. **다음 반복의 하강이나 상단에서 멈춰 있는 시간을 요구하지 않는다.** §31a의 마지막 렙 누락 원인을 실시간 경로의 필수 동작으로 복원하지 않는다.

현재 구현은 초기 출발 기준을 3개 이상·300ms 이상 안정 관측에서 만들고, 최소 진폭을 넘은 관측 2개와 복귀 방향 관측 2개·불응기를 요구한다. 이는 한 번의 큰 잡값을 왕복으로 읽지 않기 위한 초기 공학 정책이다. 정자세 판정·개인에게 필요한 운동 범위를 뜻하지 않는다. 복귀 창을 건너뛴 좌표 점프, 관측이 성긴 빠른 반복, 너무 짧거나 덜 관측된 왕복을 이후 서 있는 시간으로 살리지 않는다.

복귀 경로에서는 원표본과 방향 히스테리시스를 사용한다. 300ms 간격에서 3점 중앙값이 정지 없는 끝점 하나를 지워버리는 문제를 피하기 위한 선택이다. 과거 연구 재생용 반전 경로의 평활은 별도로 남는다. 새 손실 정책으로 과거 fixture의 재생 수가 달라질 수 있으며, 패리티를 맞추려고 누락 구간을 다시 연결하면 안 된다.

### 56.5 촬영 방향과 반대 편차는 확인할 수 없으면 유보

- `views_ok`가 있는 window 규칙에서 뷰 추정 실패·UNKNOWN·방향 정보 부재는 `ABSTAIN: 촬영 방향 · 확인할 수 없음`이다. §33의 UNKNOWN이면 제한을 생략하던 설명은 이 절로 대체한다.
- 실제 뷰가 허용 목록 밖인 규칙도 유보한다. `views_ok`가 없는 규칙과 시간/반복 전용 규칙까지 동일한 뷰 계약을 새로 가정하지 않는다.
- primary가 OK여도 `oppositeGuard`를 넘은 방향의 `validated=false`이면 `Direction.OPPOSITE`를 보존한 채 ABSTAIN이다. 이를 정상이나 검증된 ship 위반으로 바꾸지 않고 점수·음성 자격을 상속시키지 않는다.

### 56.6 실제 전달 이후 새 관측 창으로만 회복

`SpeechCoach.speak(..., onDelivered=...)`의 콜백은 TTS `onDone`에서 한 번만 실행한다. 요청, 큐 등록, `onStart`, 음소거, 오류, 중단, flush로 버린 발화는 전달 완료가 아니다. 취소된 음성의 늦은 완료 콜백과 이전 epoch/이전 코치의 이벤트를 전달 확인으로 쓰지 않는다.

`LiveCoach.onFeedbackDelivered`는 현재 이벤트·관측 epoch·단조 시각을 확인한 뒤 전달 시점과 프레임 인덱스를 저장한다. `RECOVERED`는 해당 전달 이후 새 프레임으로만 채운 유효한 최근 창과 회복 지속 조건을 요구한다. 같은 창의 evaluate 재호출로 지속 횟수를 쌓지 않는다. 가림·유보·조건 경계는 전달 확인을 무효화한다. 시간 없는 연구/랩 입력은 전달 후 순서를 검증할 수 없어 회복 이벤트를 만들지 않는다.

**이 구현은 ROM·동작 위상·부하·템포 유지나 다른 보상 움직임까지 확인한 교정 효과 검증이 아니다.** 전달 후 특정 측정 항목이 참고 범위로 돌아온 증거다. 따라서 화면/세트 요약은 ‘교정됨’ 대신 ‘관측 회복’, 전체 정상 보장 대신 ‘판정한 항목 범위 내’를 사용한다. 음성이 실제로 끝났어도 사용자가 들었거나 이해했음을 입증하지 않는다. 현재 실시간 연결은 TTS 완료이며 단순 화면 표시를 사용자의 수신 확인으로 대체하지 않는다.

### 56.7 촬영 조건이 바뀐 세트의 최종 평가

`PostureAssessment.evaluate(..., contextStartAt=...)`는 마지막 촬영 조건 시작 이후의 window·hold·정렬·rep 자료만 최종 자세 평가에 넣는다. 종료 경계에 사용하는 반복 시각도 해당 촬영 구간으로 제한한다. 코칭 요약은 마지막 연속 관측 구간을 비교한다.

완료한 총 반복 수·완료 시각 이력은 세트 전체를 보존한다. 촬영 조건 변경 전후 경계는 로그의 `context_boundaries_ms`에 남기고, 화면 설명에는 **마지막 촬영 구간의 자세 평가와 세트 전체 완료 횟수**라는 범위 차이를 알린다. 마지막 구간이 짧거나 판정 불가이면 유보다. 이전 구간의 유효한 숫자를 끌어와 마지막 촬영 조건을 정상으로 채우지 않는다.

### 56.8 스쿼트 37개 세부 이벤트와의 연결

`design/TREX_스쿼트_세부편차와교정멘트_명세.md` 및 `design/TREX_스쿼트_세부이벤트와멘트.json`은 37개 이벤트·52개 문구, 자동 활성화 0개, 임계값 null의 연구 계약이다. 진단 게이트, 정상 예외, 참조 출처, 같은 사건의 음성 억제, 발 기준 변경 시 무릎 판정 무효화, 발화 현재성, 후속 재평가를 구조화했다. 이 문구 목록은 실행 가능한 검증 완료 검출기가 아니다.

한 발 전체 들림·뒤꿈치 이격·발 가장자리 이격은 서로 다른 관찰이고, 발 옆 이동 뒤의 무릎 경로는 기존 발 기준으로 재지 않는다. 무릎 바깥 이동도 승인된 변형·발 방향·같은 깊이의 경로를 확인해야 하며 간격 증가만으로 오류가 아니다. 사건이 사라져도 가림 또는 반대 방향 과교정이면 성공이 아니다.

**MediaPipe world pose만으로 발바닥 압력 또는 신발과 실제 지지면의 접촉을 확정할 수 없다.** 발목·heel·foot index의 상승은 접촉 이격의 직접 증거가 아니다. 별도 RGB/지지면 추적을 구현하더라도 허용 뷰·해상도·가림·신발/웨지 조건별 검증이 필요하다. 현재 구현에서 해당 37개를 일괄 켜지 않는다.

### 56.9 아직 남은 검증과 구현

1. JVM 303건과 APK 빌드를 통과했다. 과거 Python 비교는 명시적 `forLegacyReplay`에서 4회, 실시간 엄격 정책은 가림을 나눈 독립 재생과 동일한 3회임을 각각 검사했다. 실제 사람 정확도를 이 숫자로 대신하지 않는다.
2. 실제 Android 기기에서 카메라 전환/회전/일시정지 경계, TTS 완료·중단·음소거, 프레임 지연/누락, 세트 마감의 스레드/라이프사이클 결합을 확인한다. 현재 실제 기기 증거는 없다.
3. 전체 26종 범위 중 좌우 교대 운동과 스탠딩 사이드 크런치의 실제 카운트는 후속 보강·검증 대상이다. 일반적인 한 신호 왕복 FSM이 좌우별/한 쌍 집계 의미를 해결했다고 보지 않는다.
4. 사람·세션을 나눈 실제 영상으로 연속 동작, 마지막 반복, 짧은 가림, 정상 변형, 일부러 만든 단일·복합 편차, 과교정, 카메라 오류를 평가한다. JVM 시뮬레이션은 실제 사람 정확도·교정 효과의 증거가 아니다.
5. 회복 검증에 같은 ROM·위상·템포·부하와 보상 이벤트 관찰을 연결한다. 현재 ‘관측 회복’을 제품 설명에서 ‘올바른 자세 교정 완료’로 승격하지 않는다.

## 57 — 좌우 반복의 단위·관측 기록·기기 통합 (2026-09-19)

교대 컬/니업을 평균 신호로 세면 좌우 역위상이 상쇄되고, 런지의 두 무릎을 따로 더하면 하나의 반복이 두 번 기록된다. `ExerciseRepProfiles`에 요청 26종의 변형·신호·수행 방식·관측 한계를 명시하고 `ExerciseRepTracker`를 실시간 화면에 연결했다. 플랭크는 기존 유지 시간 경로이며 나머지 25종은 반복 관측 프로필이다. 전체 57종 카탈로그 중 기존 추가 종목은 종전 신호의 공통 횟수 경로를 유지한다.

동시 양측 복귀는 총 1회, 좌우 각각은 측별 왕복 1회가 합계 목표에 더해진다. 런지는 미구분 전체 1회 또는 사용자가 지정한 기준 다리만 측정한다. 동시는 복귀 시각 차이 600ms 이내의 짝짓기이며 전체 궤적 동시성 인증이 아니다. 수행 방식은 계획 저장·재진입에서 유지되고 구버전/알 수 없는 값은 운동의 유효 기본값으로 처리한다. 플랭크에는 반복 enum을 적용하지 않는다.

현재 방향은 준비 중에도 예열하는 `RecentRepView`로 확인한다. 전체 세트의 과거 정면 방향을 현재 반복의 허가로 사용하지 않으며 현재 프레임과 최근 창의 불일치·결측·역행은 유보한다. 덤벨 컬 양측 방식은 준비 안내·시범·TTS·방향 확인 모두 정면으로 통일한다. 동일 공통 신호에는 기존 plausibleMin/Max를 보존해 손목 붕괴가 푸시업 복귀로 집계되는 회귀를 막았다. 새 측별 신호로 기존 ROM 임계값을 옮기지 않는다.

기존 세트 JSON에 수행 방식·관측 총수·좌우/함께/미구분 수와 측별 원시 이벤트를 추가했다. BOTH는 단일 극값을 비우고 두 측의 시각·피처·극값을 유지한다. 수동 실제 횟수와 카메라 관측은 별도이며 좌우 수를 다시 합산해 총수를 만들지 않는다. 완료·기록 화면은 ‘관측 N회 · 자세 미확정’으로 표시한다. 마지막으로 측정 가능했던 연속 구간의 템포는 관측 손실 뒤에도 최종 보고서에 남기되, 손실 간격이나 같은 프레임의 좌우 완료를 주기로 계산하지 않는다.

상세 계약과 잔여 한계는 [좌우 반복 엔진](../../docs/BILATERAL_REP_ENGINE.md)을 따른다. 스탠딩 사이드 크런치의 대표 원본 프레임과 메타데이터를 확인했으며 무릎 높이 L/R, 같은 측 무릎–팔꿈치 거리, 몸통 기울기를 사용한다. 팔만 내리는 합성 반례는 차단하지만 무릎만 올림과 정확한 몸통 수축은 아직 충분히 구별하지 못한다. 등록된 새 프로필은 전부 미검증 상태이며 기존 자세 규칙 등급과 스쿼트 37개 세부 이벤트의 활성 상태는 변경하지 않았다.

최종 검증: JVM 383건 통과(실패·오류·생략 0), 분리 평가 앱과 instrumentation APK 빌드 성공. Galaxy Note10+ SM-N976N / Android 12에서 저장 5건·카메라/세션 2건·설정 저장/재진입 2건·모델 추론 1건·한국어 TTS 1건, 서로 다른 총 11개 항목의 최종 시도가 통과했다(회차 합산). 최초 잠금화면 실패와 접근성 노드 캐시 오독은 별도로 보존했다. 카메라 종료 중 모델 해제 경합을 발견해 동일 분석 executor에서 순서대로 닫도록 수정했으며, 이후 카메라/세션 검사에서 task graph 오류가 재현되지 않았다.

실제 기기의 저장 이미지 FULL/GPU 추론 30회는 중앙값 90ms·95백분위 100ms, TTS 완료 콜백은 1회였다. 저장 JPG 반복 검사이므로 실제 운동 정확도·CameraX 전체 지연·음성 청취/이해·장시간 발열의 증거가 아니다. 최신 평가 앱 설치 SHA-256 일치와 일반 앱 설정 3개 파일의 바이트 보존을 확인했다. 상세 수치·빌드별 출처는 [좌우 반복 엔진](../../docs/BILATERAL_REP_ENGINE.md)과 Codex 작업 산출물 `outputs/engine-bilateral-device/verification.json`을 따른다. 실제 사람의 횟수 정확도·교정 효과는 미측정이다.

## 58 — 평가 앱 식별·실행 엔진 근거·기록 재생 (2026-09-19)

평가 앱의 launcher 이름은 `TREX 엔진 평가`, 버전 접미사는 `-engine-eval`이다. 일반 앱과 같은 표시 때문에 실행 대상을 구분할 수 없던 문제를 해결한다. 운동 목록·준비·HUD에 평가 상태를 표시하되 새 횟수 엔진과 기존 자세 규칙을 같은 ‘전면 신규 엔진’으로 표현하지 않는다.

`EngineProvenance`는 실제 생성된 tracker·profile·pattern과 앱 패키지·자세 규칙·관절 추론 계층을 기록한다. `PostureLive`의 진단 로그와 세트 JSON 최상위 `engine`가 같은 정보를 쓰며, null인 과거 로그에는 엔진 필드를 추가하지 않는다. 반복은 `return-bilateral/1`, 요청 운동 프로필은 `exercise-rep-profiles/1`, 규칙 평가는 `rule-coach/1`, 관절 추론은 기존 MediaPipe FULL이다. 유지 운동·수동 기록도 별도로 구분한다.

`RecordedRepReplayTest`는 실제 기기에서 저장한 피처·시각·결측을 현재 Kotlin 엔진으로 재생하고 프레임별 관측 유보와 완료 이벤트를 출력한다. 현재 확보한 26세트·3,085프레임에는 사용자 확인 횟수 정답이 없으며, 이전 카운터 출력을 정답으로 복사하지 않는다. AIHub 합성 시각의 카운트 일관성을 실제 속도·정확도로 읽게 하던 `REP_SIGNALS.md` 설명도 정정했다.

JVM 386건(실패·오류·생략 0)과 평가 앱·instrumentation 빌드를 통과했다. 상세 실행 계약과 증거 위치는 [평가 앱 실행 엔진](../../docs/ENGINE_EVALUATION.md)을 따른다. 새 버전 표시 자체는 횟수 정확도나 세부 자세 교정 구현 완료의 증거가 아니다.

## §59 — 독립 엔진 실험 앱 (2026-09-19)

사용자 요청으로 이식 전에 독립 앱 검증을 진행한다. `engine-lab/`은 별도 Gradle 루트이며 Android UI `:app`과 순수 Kotlin `:engine`만 빌드한다. 설치 패키지는 `com.trex.engine.lab` / 이름은 `TREX 실험실`이다. 기존 제품 앱의 LiveCoach·PostureRuleSet·RepCounter·규칙 JSON은 포함하지 않는다. 새 복귀/좌우 카운터와 26종 프로필을 분리했고, 관절 수학·MediaPipe 자산의 재사용 출처를 남겼다.

현재 실행 범위는 25종 왕복 관측·플랭크 관측 시간·종목별 관절 기하 측정이다. 147개 설계 항목의 자동 판정/교정은 미구현이며 화면·로그에서 분리한다. 원본 좌표·피처·반복 이벤트·보류 사유·엔진/모델 해시와 수동 정답을 저장하고 ZIP 공유한다. 사용자의 지시 횟수를 정답으로 기록하지 않는다. 회수·분석 및 재생 명령은 `engine-lab/README.md`가 정본이다.

JVM 74건, 실기기 독립성/모델/로그 검사 3건 통과. 실제 앱 촬영 로그의 원시 관절 재생 검사를 추가했다. UI 검사는 휴대폰 잠금 해제 후 수행하며 최종 기록은 `outputs/engine-lab/verification.json`에 남긴다. 사람의 반복 정확도·자세 판정 정확도·교정 효과 검증은 별도다. 독립 앱 실험 후 이식한다.


## §60 — trex_v2 제품 연결과 26종 관측 계약 (2026-09-20)

사용자 요청으로 `codex/trex-v2-engine` 브랜치에 기존 앱 UI와 공유 엔진을 연결했다. `:pose-engine`은 `engine-lab/engine` 원본을 직접 사용한다. 실제 실행은 `PostureV2Live` → `PoseEvaluationEngine`이며 구형 LiveCoach/규칙/기준선 감산 경로를 실행하지 않는다. 그 이유는 새 사람 검증 없이 스튜디오 규칙을 개인 교정 명령으로 승격시키지 않기 위해서다.

새 선택은 AIHub 고유 26종, 이전 별칭은 정규화, 과거 기록은 보존한다. 시작/복귀·보조 동작·가시성·관측 경계와 명시적인 범위 목표를 분리한다. 자동 횟수는 참고 관측이며 직접 수정값을 별도로 보존한다. 목표가 충족되어도 자동으로 세트를 끝내지 않는다. 플랭크는 시간만 관측한다. 형식적 전체 자세 점수·정상 인증·행동 지시 음성은 없다. 진단 좌표는 기본 꺼짐이며 no_backup 공간의 7일/20MB 한도를 사용한다.

설치 패키지는 `com.example.trex_kotlin.v2`, 이름은 `trex_v2`다. 기존 일반 앱은 보존한다. 구현 범위·근거·검증·미제공 항목의 정본은 [TREX_V2_IMPLEMENTATION.md](../../docs/TREX_V2_IMPLEMENTATION.md)와 `outputs/v2_implementation_20260920/verification.json`이다. 합성 시험과 기존 이미지 추론을 사람의 운동 정확도로 해석하지 않는다.

## §61 — 사용자 요청에 따른 v2 교정 음성·참고 점수 활성화 (2026-09-20)

사용자가 “교정음성과 점수 활성화해”라고 요청했다. §60의 기본 미제공 정책을 변경하되, 관측하지 않은 것을 정상으로 세거나 이미 확인한 대리 피처의 오해를 재도입하지 않는다. 공유 횟수 엔진은 유지하고 앱의 `V2FormFeedback` 정책 계층에 기존 ship/window 규칙을 연결한다. 구형 기준선은 자동 감산하지 않는다.

- 26종 중 16종 30개 고유 피처 규칙을 점수·음성 후보로 사용한다. `knee_out_mean`·`palm_lat`·`stance_w`는 제외해 스쿼트/데드리프트 무릎, 바벨 밀착, 행잉 다리 모음 오해를 되살리지 않는다. 데드리프트·사이드 런지·바닥 8종에는 적격 규칙이 없으며 검증 중을 표시한다.
- 기존 조건명 대신 측정 이름을 쓴다. 예: 척추 중립 → 몸통 기울기, 팔꿈치가 손목 리드 → 양손 높이 차이. 음성은 참고 범위 이탈과 천천히 움직이며 해당 항목을 확인하라는 안내다. 자세의 원인·특정 방향 처방·척추 곡률을 추정하지 않는다.
- 준비 이후 첫 완결 반복이 앵커다. 서로 겹치지 않는 8프레임 창 두 개에서 판정과 방향이 일치해야 반영한다. 가림·시간 역행·1초 초과 공백·촬영 변경은 현재 창을 폐기하고 새 앵커를 요구한다. 음성은 항목별 12초/전체 4초 간격이며 교정 중 횟수 발화를 억제한다. 음소거·정지·기록 모드는 교정 음성을 내지 않는다.
- 실시간 참고 점수는 현재 창의 범위 내/판정 항목 수 × 100이다. 세트 점수는 세트 중 안정적으로 관측한 이탈을 보존한다. 현재 점수와 세트 점수는 집계 범위가 다르다. beta/exclude/ABSTAIN은 분모에서 제외하며 판정 0건은 null이다. 항목 수를 항상 드러내고 전신 정확도가 아님을 표시한다.
- 종목별 업데이트 첫 진입은 COACH, 음성은 켜짐이다. 이후 준비 설정에서 TRACK 선택과 음소거를 제공한다. 요약 로그에는 실제 모드의 점수/판정/음성 설정 및 `v2-feedback/1` 정책과 항목별 결과를 기록한다. 음성 설정값은 전달 성공 건수가 아니다.

이는 기존 AIHub 스튜디오 임계값을 제한적으로 다시 연결한 변경이며 신규 데이터 학습·미보정 규칙 승격·사람의 교정 정확도 검증이 아니다. 자세 기준의 출시 타당성과 촬영 환경 일반화는 별도 검증해야 한다. 실제 빌드·기기 검증은 [구현 문서](../../docs/TREX_V2_IMPLEMENTATION.md)를 따른다.

## §62 — v2 브랜치 README와 APK 배포 준비 (2026-09-23)

사용자가 `main` README는 유지하고 `feature/pose-evaluation-engine`에서 이 대화의 두 번째 버전 APK를 안내하도록 요청했다. §60·§61의 `trex_v2`를 기준으로 첫 버전 비교와 고유 26종 지원표를 README에 작성한다. 25종 반복·플랭크 관측 시간과 16종 30개 교정/점수 후보를 분리하며 실험실 APK로 대체하지 않는다.

`v2.0.1-feedback-preview` 사전 릴리스에는 소스 커밋과 일치하는 APK 및 SHA-256/빌드 정보를 제공한다. 개인 작업 폴더의 미추적 이미지·로그가 공개 APK에 섞이지 않도록 커밋만 있는 별도 체크아웃에서 빌드한다. 기존 첫 버전 릴리스는 수정하지 않는다. 실제 검증·다운로드 정책은 [ANDROID_V2_RELEASE.md](../../docs/ANDROID_V2_RELEASE.md)를 따른다.

## §63 — 수집·독립 정답·동일 좌표 재생 평가 앱 (2026-09-23)

사용자 요청으로 [VALIDATION_STUDIO.md](../../docs/VALIDATION_STUDIO.md)를 정본으로 설계와 구현을 진행했다. 이유는 영상·원본 좌표만으로는 독립 정답, 실제 시간축, 엔진 버전, 유보의 분모 및 반복 누락/추가를 재현할 수 없기 때문이다.

- `-PvalidationStudio` APK의 런처는 검증 화면이며 일반 v2 빌드는 기존 진입점을 유지한다. 카메라 앱 촬영/SAF 영상 가져오기 → 앱 원본 복사 → CPU VIDEO 추론을 수행한다. 디코더 출력 PTS에 따라 최소 200ms 간격으로 표본을 뽑고 실제 PTS를 보존한다. 입력 요청 시각/합성 FPS를 정답 시각으로 쓰지 않는다.
- 추출·정답 개정·재생은 불변 실행 ID와 해시로 연결한다. 미완료 추출은 채점 입력에서 제외한다. 모델 원본 visibility/presence와 적용 후 가시성, world/xy/z를 분리하며 좌표는 모델 출력으로 명시한다. 연구 자료는 자동 만료·클라우드 백업에서 제외한다.
- 사람·촬영일·운동 변형·촬영 방향을 기록하며 수행자 간 train/calibration/test 분할 충돌을 차단한다. 반복 정답은 명시적 검토 전에는 null이다. 항목별 정답은 미입력·판단 불가·정상·오류를 구분한다. 예측 공개 이후의 정답 수정은 비맹검으로 표시한다.
- 구형 태그 `3322628a2c58ab3d02266d6e9e824ffb12bfd616`의 반복 코어와 v2 제품 엔진을 동일 좌표로 재생한다. 자세 기준선은 공통 ship/window 평가이며 구형 APK의 전체 처리나 음성을 복제하지 않는다. 원본 v2 음성·점수 정책은 이 작업에서 변경하지 않았다. 읽기 전용 판정 근거 진단만 추가했다.
- 채점은 순서 보존 일대일 반복 매칭(초기 허용 오차 500ms), 유지 구간 교집합, 라벨 시간별 정상 오탐·오류 검출·판정 범위다. 유보와 미지원은 정상으로 채우지 않으며 예측은 다음 표본 또는 250ms까지만 이어진다. 최종 시험 결과를 본 뒤 튜닝하면 새로운 시험 데이터가 필요하다.

검증은 JVM 계산 계약과 Android 합성 MP4·실제 모델/파일/화면을 구분한다. 실제 사람 정확도·영상 프레임별 절대 UTC·실시간 발열/지연·음성 전달 후 교정 효과는 이번 앱의 입증 범위가 아니다.
