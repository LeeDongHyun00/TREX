...
`docs/REP_ENGINE_DESIGN.md` 가 정본이다. 외부 코퍼스 재생(`research/external_rep_replay/README.md` §4·§7)으로 현재 라이브 카운터
(`ReturnRepTracker`)의 놓침 원인이 촬영이 아니라 네 가지 설계 선택(런지 신호 `knee_out_mean`, 안정 준비 자세 요구, 상단 체류 요구,
3점 중앙값 평활)임을 확인한 뒤, 엔진을 새로 만들지 않고 그 네 곳을 바꾸는 설계다.

- 신호 규칙: 회전 불변 관절각 우선, 교대 종목은 좌우 개별 카운터 + 500ms 병합(한쪽 1회 = 1회), 게이트는 물리 단위·잡음 바닥 이상.
  런지 → `knee_mean` 35°, 덤벨 컬 → `elbow_L`/`elbow_R` 각각.
- 카운터 코어: 복귀 히스테리시스(휴식 → 하강 → 반전 → 75% 복귀에서 확정, 재하강 폴백, 불응기 0.8s, 평활 없음). 마지막 반복은 다음 하강 없이 확정(§42 유지).
- 세트 시작 확정: 첫 두 사이클을 진폭 일관성으로 함께 확정 — 준비 자세 앵커를 대체한다.
- 신뢰 등급(확정/감지 중/저신뢰)을 HUD·진행 조건·음성에 연결하고, 반복당 샘플 < 5 또는 발열 감속이면 저신뢰.
- ROM 은 카운트와 분리 유지, 원값으로 판정, 교대는 쪽별. 기준값은 MM-Fit 에서 뽑지 않는다(원칙 #4).
- 프로토타입 실측(MM-Fit 자체 3D 포즈, 300ms): 세트 정확 일치 스쿼트 0.53 → 0.97, 런지 0.00 → 0.95, 컬 0.42 → 0.98, 세트 안 과다 카운트 0.
  남은 약점은 컬의 세트 직전 헛카운트(0.22/세트, 게이트 50° 면 0.12) — 휴대폰 라벨 세트에서 확정한다.
- 검증 사다리: MM-Fit 3D(완료) → MM-Fit 영상 MediaPipe → REHAB24-6 경계 F1 → 휴대폰 자가 라벨(ship 게이트). 마지막 전까지 beta.
- (추가) 잡음 스트레스(§9): 발열 감속(433~466ms)만으로 현재 엔진은 스쿼트 재현율 0.90 → 0.32, 제안 코어는 0.997 유지. 팔별 팔꿈치 신호는 프레임별 σ ≥ 10° 부터
  과다 카운트 — 50° 게이트와 잡음 적응 평활이 후보. "잡음으로 학습" 은 정답 부족·잡음 모델 미측정·귀속 불가로 지금은 보류, 사이클 게이트 분류기가 학습이 값을 할 첫 자리.
