...
## §56 — AIHub 연결 26종목으로 서비스 범위 제한 (2026-09-23)

- 기준 브랜치: feature/posture-coach-reliability, 5237ba9. 기존 57개 카탈로그에서 미연결 30개와 바벨 스쿼트 규칙을 공유하던 기본 스쿼트를 제거했다. 서로 다른 AIHub 종목 26개(서서 18, 바닥 8)만 선택·추천·실행한다.
- ExerciseProfiles가 지원 이름/참조의 정본이며 postureExerciseMap도 여기서 생성한다. 개발용 랩/기준선 선택도 같은 범위를 따른다. 데이터셋 전체가 26개라는 의미나 26종목의 모든 자세가 검증됐다는 의미는 아니다.
- 신규 기본 계획은 플랭크·런지·니 푸쉬업이다. 기본 스쿼트를 바벨 스쿼트로 자동 변환하지 않는다(기구·운동 의미가 다름).
- 구형 실행 계획은 원문 before_aihub26_plan을 보관하고 미지원 항목/대체안을 제거한다. 전부 미지원이면 빈 계획으로 남긴다. 기존 운동 기록·자가 라벨·기준선 파일은 보존한다.
- 연구 원본과 규칙 자산의 다른 종목은 연구 재현용으로 보존한다. 앱 서비스 진입만 26개로 한정하며 규칙 임계값과 등급은 변경하지 않는다.

## §57 — 운동 이해·방향별 교정·즉시 계수 설계 (2026-09-23, 미구현)

사용자 요청: 26종목 운동 중 실제 행동을 파악하고 잘못된 동작/자세에 구체적으로 지시하며, 완료시점에 횟수를 표시하는 서비스 설계.

- 정본 제안: [운동 이해 엔진](../../docs/POSTURE_MOTION_ENGINE_DESIGN.md), [26종목 단계·계수·교정 계약](../../docs/POSTURE_MOTION_26_EXERCISES.md).
- 확인한 현행 구조: 300ms 추론 간격과 발열 배수, 공통 1.2초 재계수 제한, 복귀 2표본/150ms, 좌우 평균 피처, 조건명 regex 발화. 바깥 무릎 문구/가드는 이미 있으므로 문구 부재로 단정하지 않는다. knee_out은 발끝 방향을 직접 재지 않는다.
- 제안: 촬영 품질→준비/운동/다른 동작→단계 추적→반복 완료와 방향 사건→발화/표시/복귀 확인. 사용자 좌우·지지발·동작 단계를 보존하고 근거 없는 해부학적 원인은 말하지 않는다.
- 관측 반복 수와 지정 기준 충족 수 분리. UI/음성 지연 목표는 제안이며 달성한 성능이 아니다. 시간 라벨 없는 AIHub 이미지로 실제 빠른 반복 정확도를 인증하지 않는다.
- 이번 작업은 설계 문서만 추가. 카운터·음성·임계값·모델·APK를 변경하지 않았으며 사용자 증상에 대응하는 새 실기기 로그는 검증하지 않았다.

## §58 운동 이해·교정 엔진 M1 구현 (2026-09-23)

- 상세 정본: [M1 구현과 검증 범위](../../docs/POSTURE_MOTION_ENGINE_IMPLEMENTATION.md). §57 설계의 첫 실행 단계이며 임의 행동 전체/모든 오류의 인증이 아니다.
- `MotionContracts` 26개: 동적 25종목은 `MotionRepCounter`, 플랭크는 기존 유지/정렬 경로. 주/보조 신호, 계수 끝점, 양측/교대/가시 측 정책과 문구를 한곳에서 정의한다. 고정 1.2초 제한과 다음 반복을 기다리는 완료 방식은 새 세션에서 사용하지 않는다.
- 요청 간격 80ms, FULL 모델·열 보호 유지. 규칙 집계는 별도 최소 300ms 표본. 카메라 단조 타임스탬프로 간격을 재고, 긴 누락·카메라/회전/일시정지 경계에서 미완성 사이클을 끊는다. 이것이 실제 12.5fps나 300ms 반응속도를 보장한다는 뜻은 아니다.
- 새 `knee_track_L/R`는 중력 수평면에서 뒤꿈치→발끝 축에 수직인 무릎–발목 위치/종아리 길이. 해당 다리의 바깥이 양수. 발축·필요 관절 미확인 때 유보한다. 표시 컷/지속시간은 잡음 억제 초기값이며 판정 임계값으로 검증되지 않았다. 기존 `knee_out` 임계값을 이식하지 않는다.
- `EvidenceCues`는 조건명의 고정된 방향 문구보다 실제 측정 의미를 우선한다. 무릎의 기존 측방 위치만으로 발 기준 안/밖 교정을 지시하지 않는다. 고개·상체·각도·거리·변동을 설명하며 척추 곡률/발 압력/안구 시선을 직접 측정했다고 말하지 않는다.
- 교대 동작은 `SideMotionComparison`으로 좌우 기준을 분리. 가동범위 기준은 계수와 분리하고 양측 평균 임계값을 단측 동작에 재사용하지 않는다. 관측/기준 충족/미달/미확인 수가 UI·리포트·영구 기록·JSONL에서 일치하도록 연결했다.
- 검증된 촬영 방향을 요구하는 규칙은 방향 미확인도 ABSTAIN. 바쁜 음성 채널에 오래된 숫자를 쌓지 않고, 실제 발화 불가 상태에서 규칙 쿨다운을 소비하지 않는다.
- 세트 로그에 `motion.version/frames/dropped`, `reps.unknown/side`를 추가. 고속 추적 피처는 6,000프레임 상한, Float 왕복 보존. `MotionReplay`와 `export_motion_replay.py`가 동일 Kotlin 계수 시점 재생을 지원한다. 구형 로그의 미관측 프레임은 복원하지 않는다.
- JVM 시험은 합성 반복/기하·상태 전이·좌우·가림·로그·기존 회귀를 검증한다. 새 실기기 반복/영상 라벨 입력이 없으므로 사용자별 정확도, 26종목 출시 성능, 지연 p95는 미측정이다. 특정 사용자의 기록으로 모집단 파라미터를 변경하지 않았다.

## §59 학습형 행동 이해 모델·휴대폰 자원 설계 (2026-09-23, 제안)

- 상세 정본: [학습형 행동 이해 모델 설계](../../docs/POSTURE_LEARNED_ACTION_MODEL_DESIGN.md), [ADR-0001](../../docs/adr/0001-causal-posture-action-model.md). §58 구현을 기반으로 학습 모델의 구조·데이터·자원·출시 판정 조건을 구체화했다. 앱/runtime/모델 가중치는 이번 설계에서 변경하지 않았다.
- 기본 후보는 MediaPipe 관절/피처 위 작은 causal TCN. 매 관측의 빠른 단계·측 경로와 최대5Hz의 행동 문맥 경로를 분리하고 과거 상태를 캐시한다. 선택 종목을 공통 행동 인식의 입력 정답처럼 주지 않는다. 부위·좌우·방향 관측과 운동 적합성은 별도 출력/계약이다.
- TARGET/KNOWN_OTHER/AMBIGUOUS/NOVEL/UNOBSERVABLE/UNAVAILABLE를 분리한다. 낯선 행동은 자세 오류가 아니고, 모델 장애는 모델이 낯설다고 판단한 결과가 아니다. 검증된 범위에서 M1 후보를 중재하며 기존 관측 반복/ROM 미확인과 새 행동 미확정도 구별한다.
- 신규 PosePacketV2는 world33, visibility/presence 각각, 실제 시각·마스크·촬영 세대를 전달해야 한다. 현재 `PoseSample`/`motion-1` 일부 피처 로그만으로 이 입력을 복원할 수 없다. 기기 IMU는 카메라 움직임 근거이며 몸에 착용한 센서가 아니다.
- 후보 예산은 INT8 모델2MiB, 추가 head/전처리 p95 5ms, runtime 포함 추가 peak PSS32MiB, 동일 M1 대비 에너지 증가10% 이내다. 달성 결과가 아니다. MediaPipe와 LiteRT 동시 로딩, CPU/GPU 경쟁, 30분 지속 성능을 먼저 계측한다. FULL/LITE 전환은 보정/규칙 포함 검증 번들 단위로만 한다.
- AIHub sparse clip은 공간·조건 약한 라벨로 활용하고 실제 반복 경계·생활 행동·가림·방향은 폰 연속 데이터로 보완한다. 사람/촬영 파생본 분리, 별도 보정/test, 행동 계열을 남긴 unknown 시험이 필요하다. 이 사용자의 기존 로그는 검증 전용으로 유지한다.
- 학습 모델 도입 순서는 입력/라벨 계약→pilot→shadow→문맥 억제→단계→항목별 방향 교정이다. 모든 임의 행동/26종목 모든 오류를 이미 인식한다고 약속하지 않는다. 음성은 정밀도·방향 오지시·recall/coverage·기기 예산을 통과한 항목만 승격한다.

## §60 학습형 운동 종류 모델·시험 실행·APK (2026-09-23)

- 정본: [구현·학습·검증·한계](../../docs/POSTURE_ACTION_MODEL_IMPLEMENTATION.md), [집계 수치](ACTION_SHADOW_RESULTS.json). 새 브랜치 `codex/posture-action-model-preview`는 §56~§59 미커밋 작업도 포함하며 배포 버전은 `1.2.0-preview.1`(code 4)다.
- `PosePacketV2`는 원래 MP world33(m/y-down), 2D, visibility/presence, 캡처 ns, 영상 크기·세대·중력 정보를 별도로 보존한다. 기존 규칙용 cm/y-up 좌표계를 바꾸지 않는다. `ActionInputBuilder`의 132차원 정규화는 Python과 합성 입력으로 파리티 검증한다.
- AIHub MP 캐시, 사람 66/22/22 분할, 27분류(26종목+기타). sparse 16프레임의 과거 구간으로 Dense48 + residual causal Conv1D(1/2/4)를 학습했다. 실제 반복 시각/생활 행동/자세 오류/피로는 학습하지 않았다. 개인 기기 로그는 사용하지 않았다.
- 가중치 28,587개, INT8 내부·float I/O TFLite 42,672바이트. 모델·FULL pose 해시/스키마/권한을 확인하는 CPU Interpreter(LiteRT 1.4.2)로 실행한다. 선택 종목은 모델 입력이 아니며 추론 후 대조에만 쓴다. 5Hz 상한·작업자 1개·대기열 누적 없음·MODERATE 발열 시 중단·시각/세대 경계 초기화를 적용한다. 더 큰 모델의 상태 캐시와 CompiledModel은 후속이다.
- 내부 test 6,192구간 정확도 75.21%, 클래스 recall 평균 79.28%. 학습하지 않은 3운동(test 사람, 475구간)의 62.11%를 서비스 운동으로 오수용했다. 자동 중재 승격 실패: 모든 상태의 `canAffectCount/canSpeak=false`, 점수/교정/계수는 기존 M1만 사용한다. test로 임계값을 다시 조정하지 않는다.
- 새 무릎 방향 관측은 beta 참고 표시/강조만 사용하고 음성 연결은 제거한다. 기존 ship/beta/exclude 자산과 모집단 임계값은 보존한다.
- 준비/일시정지의 관절 기록은 opt-in, 기본 false. 원관절 최대600프레임·예측 최대1,200건과 누락 수를 세트 로그의 `action`에 저장한다. 해제는 진행 중 원자료를 비우며 이미 저장된 로그 삭제를 뜻하지 않는다. 자동 전송/영상 저장 없음. 예측 로그는 수집 스위치와 별도로 남는다.
- `export_action_shadow.py`는 새 스키마·명시적 수집·단조 시각·누락 여부를 검사하고 입력만 내보낸다. 기존 300ms 피처 로그를 원관절로 복원하거나 선택 종목/예측을 정답으로 쓰지 않는다. 공개 시험 픽스처는 합성 관절이며 실제 사람 자료/ID는 outputs에만 보관한다.
- JVM 310통과/1조건부 건너뜀, Python 5통과, API36.1 x86_64 가상 기기 모델2건 통과. APK 설치·최초 실행·기존 인증서 일치 확인. 실제 사람 연속 동작, ARM 기기, 지연·PSS·전력·30분 발열 예산은 미검증이다. 최신 다운로드/해시는 [배포 문서](../../docs/ANDROID_PREVIEW_RELEASE.md)를 따른다.
