...
## §56 — iOS 자세 기능 확장을 위한 KMP 공통 엔진 설계 (2026-09-12, 구현 전)

- 사용자 요청은 iOS 확장을 위한 **구현 전 설계**다. 정본 설계는 [ADR-056: iOS 자세 기능 KMP 설계](../../docs/IOS_POSTURE_KMP_DESIGN.md)이며, 분석 기준은 `feature/posture-coach-reliability`의 `5c7118e`다. KMP 모듈·Swift 앱·새 API는 아직 구현하지 않았다.
- 결정 제안: Kotlin 계산과 세션 상태를 `:posture-core`에 모으고, Android는 기존 Compose/CameraX, iOS는 SwiftUI/AVFoundation 및 iOS MediaPipe를 사용한다. C++ 재작성·모델 교체·UI 전체 공통화·임계값 변경은 첫 이식 범위에서 제외한다. Kotlin 후처리 독립 실측이 없어 성능 개선을 전제로 선택하지 않는다.
- 공통 범위는 피처·규칙뿐 아니라 준비 프레임 제외, 앵커, 반복 생성 인자, 가림/일시정지, 개인 기준, 촬영 구간, 모드·음성 자격, 최종 마감까지다. 현재 `PostureLive.kt`와 앱 진행기의 정책을 각 OS에서 따로 재작성하면 같은 계산식을 써도 결과가 달라지는 것이 근거다.
- 입력 계약은 원본 MP 33점의 좌표/신뢰도·회전 보정 이미지 크기·엔진 좌표계 up·단조 시각·세트/단계/촬영 token을 포함한다. 추론 시작 전 token을 붙이고 늦은 준비/이전 세트 프레임을 폐기한다. 프레임·조작·마감을 직렬 처리하며 같은 결과의 저장/음성 효과는 중복 실행하지 않는다.
- 자산은 현행 Android assets를 초기 원본으로 삼아 양쪽 번들 해시를 맞춘다. v1 피처 로그는 보존하되 원본 관절/조작 재생은 별도 `replays/` 형식으로 설계한다. iOS/복수 촬영 구간 로그를 기존 재보정 도구가 자동 합치지 않도록 입력 선택을 명시한다. `android_video_training` 정상 표본은 iOS 검증 기준으로 승격시키지 않는다.
- 현재 바닥/세트 요약의 beta 음성과 AGENTS의 beta 침묵 지침은 충돌한다. 목표 정책은 교정·위반·복귀 음성을 차단한다. 카메라 구간 분리·단조 시각·부분 관측 표시와 함께 설계 §12에 **기존 동작과의 의도적 차이**로 기록하며, 수치 계산 추출과 별도 검증한다. 이 문서는 지침을 완화하거나 기존 앱을 수정한 기록이 아니다.
- 이전 순서: P0 기준 픽스처/성능 확보 → P1 계산 모듈 추출 → P2 공통 세션/정책 → P3 iOS 검증 앱 → P4 플랫폼 회귀/실촬영 → P5 전체 iOS 제품 연결. 같은 입력의 JVM/Native 동등성과 iPhone 촬영 정확도·TTS 사용성은 서로 다른 검증이다. 초기 300ms/8프레임 정책을 보존하며 성능 목표는 미측정 제안값이다.
- **이번 검증 범위는 문서의 소스 대조·링크·차이 검사다. 앱 코드·Gradle·규칙·모델·APK는 변경하지 않았고 빌드·테스트·기기 설치는 실행하지 않았다.** Mac/Xcode·지원 iPhone 확인과 실제 이식은 후속이다.
...
- Windows 내용 검사 8개/규칙 분포 통과, strict에서 텍스트 7개의 CRLF 차이 검출. 임시 Git 원본 바이트의 strict 통과 및 변경/누락 파일 거부를 확인했다. Android 앱·Gradle·규칙·모델은 변경하지 않았으며 Android/Native 빌드·카메라 검증·P0 전체·P1 구현은 미완료다.
