...
object ViewGuide {
    ...
    fun placement(view: String, floor: Boolean, mirrorSafe: Boolean = true): String {
        ...
        // §33: 좌우는 사용자 기준 — B 는 오른어깨가 카메라에 가까운 배치(사용자 오른쪽 앞), D 는 왼쪽 앞.
        // 데이터셋의 "전방사선L" 은 카메라 쪽에서 본 왼쪽이라 사용자에게 "왼쪽" 이라 하면 거울처럼 반대로 놓는다.
        val side = if (mirrorSafe) "좌우 어느 쪽이든" else if (view == "B") "사용자 기준 오른쪽" else "사용자 기준 왼쪽"
        val dir = when (view) {
            "C" -> "몸을 정면에서 마주보게"
