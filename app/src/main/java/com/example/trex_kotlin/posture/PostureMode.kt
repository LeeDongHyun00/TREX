 ...
 *      · 음성 = 렙 카운트 + PostureComparisonTracker의 동일 동작 단계 대비 지속 변화·복귀.
 *      · 모집단 규칙의 정상→위반 전환은 개인 변화량으로 사용하지 않는다(§38).
 *      · 무효 렙 → "파셜"로 개명·집계만: 숙련자의 파셜은 훈련 기법이지 잘못이 아니다. 단 '파셜' 은 검증된 ROM 기준
 *        (`RepRomTier.VALIDATED`)에서만 — 미검증 기준은 '참고 · 범위 미달', 기준 없음은 '범위 미판정'(spec §58).
 *      · 붉은 위반 강조 대신 변화 부위를 호박색으로 표시. 초반/현재 수치와 템포를 보여 준다.
 *
