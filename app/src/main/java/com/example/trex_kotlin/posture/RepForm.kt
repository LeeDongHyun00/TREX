    ...
    private fun squat(): List<RepFormCheck> {
        ...
        return listOf(
            ...
            RepFormCheck("repform|$ex|발 간격|시작", ex, "발 간격(시작)", "발 너비", RuleStatus.BETA, Stance2d.FEATURE, RepPhase.START, RepFormStat.MEDIAN, RepFormRef.NONE,
                lo = 0.7f, hi = 1.6f, lowText = "발이 어깨보다 좁아요", highText = "발이 어깨보다 많이 넓어요", lowLabel = "좁음", highLabel = "넓음",
                ...
                cautions = listOf(PROVISIONAL, "넓은 스탠스(스모)는 정당한 변형일 수 있다 — 세트 전 안내로만", "폰이 낮으면 발목이 어깨보다 가까워 비율이 커진다 — 절대 띠는 넉넉히")),
            RepFormCheck("repform|$ex|발 간격", ex, "발 간격(반복)", "발 너비", RuleStatus.BETA, Stance2d.FEATURE, RepPhase.STANDING, RepFormStat.EXTREME, RepFormRef.START_RATIO,
                lo = 0.7f, hi = 1.4f, lowText = "발 너비가 시작보다 좁아졌어요", highText = "발 너비가 시작보다 넓어졌어요", lowLabel = "좁음", highLabel = "넓음",
                ...
                cautions = listOf(PROVISIONAL, "한 사람 한 세트의 불변성 — 다음 세트에서 재확인", "시작 자세 5프레임 하나가 기준이라 그 순간이 평소와 다르면 세트 전체가 어긋난다(12:19: 시작 1.10 vs 끝 0.91~1.03)")),
