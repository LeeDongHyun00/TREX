...
class ExerciseRepTracker(
    ...
    private fun counter(signal: RepSignal?) = signal?.let {
        RepCounter(it, refractoryMs = refractoryMs, maxGapMs = maxGapMs, completeOnReturn = true)
    }
    ...
    private fun advance(counter: RepCounter, timeMs: Long, features: Map<String, Float>): RepRecord? {
