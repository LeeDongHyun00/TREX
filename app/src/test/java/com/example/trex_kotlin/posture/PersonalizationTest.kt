...
class PersonalizationTest {
    @Test fun everyCatalogExerciseHasOneExplicitProfile() {
        val names=com.example.trex_kotlin.workoutCatalog.values.flatten().map { it.name }.toSet()
    ...
    @Test fun guidedBundlesDoNotBorrowUnrelatedRulesAndVariantsStaySeparate() {
        ...
        assertEquals(ExerciseProfiles.forName("바벨 스쿼트")!!.name,ExerciseProfiles.forName("기본 스쿼트")!!.name)
    ...
    @Test fun stableHipCanBeCollectedWhileHeadMoves() {
        ...
        for(ms in 0L..10000L step 250) t.add(ms,mapOf("hip_dev_ankle" to .1f,"head_trunk_ang" to if(ms%500==0L)0f else 40f))
        assertEquals(listOf("hip_dev_ankle"),t.snapshot.values.map { it.metric.feature })
    }
