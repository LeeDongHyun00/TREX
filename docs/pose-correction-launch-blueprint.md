...
현재 첫 구현 slice는 metadata-only dataset snapshot과 순수 Kotlin scalar criterion core까지다. criterion core는 명시적 phase window, 사용 가능한 시간 coverage, 품질 evidence mass, 보정된 상관시간으로 제한한 유효 표본수, 정확히 일치하는 calibration contract·artifact content SHA·evaluator-spec SHA, `PASS/FAIL/UNKNOWN` 포함관계를 구현한다. evaluator-spec SHA는 target interval과 required capability까지 묶는다. 아직 Gold calibration artifact와 candidate/person-lock 경계가 없으므로 기존 운동 cue에는 연결하지 않는다. 단위 테스트 통과는 10번 criterion engine의 수학 코어 증거일 뿐 M2 완료 증거가 아니다.
