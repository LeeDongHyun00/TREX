...
이 문서는 별도 `feature/foldable-ux` 브랜치에서 작성한 조사 자료다. 아래 코드 위치와 조사 당시 상태는 역사적 근거로 보존한다. 현재 배포 기반 브랜치에 적용한 범위와 검증 결과는 [UI 복구·폴더블 적용 기록](UI_RESTORATION.md)을 정본으로 본다. 물리 폴더블 센서 검증은 수행하지 않았다.
