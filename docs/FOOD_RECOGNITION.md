...
- **클래스별 최고 confidence, NMS 없음.** 임계값 0.40 이상인 **모든 클래스**를 돌려주므로 서로 다른 음식은 한 사진에서 최대 5종까지 나온다. 박스 좌표를 버리기 때문에 못 하는 것은 위치·같은 음식의 개수·공간적 중복 판정이며, 수량은 사용자가 스테퍼로 맞춘다. (2026-09-21 문서에 "하나만 잡는다"고 잘못 적었다가 Astra 리뷰로 정정 —  참조.)
