...
- **임계 미만 후보를 버리지 않는다(2026-09-23).** 0.40 미만은 결과에 넣지 않지만 `FoodDetectionResult.Success.candidates` 로 따로 돌려 "빠진 음식 추가"에서 고를 거리로 보여준다. 이 모델은 **"음식 없음"을 배운 적이 없어**(학습 이미지가 전부 음식 1개짜리다) 무엇을 넣든 늘 무언가를 뱉으므로, 놓친 음식이 임계 바로 아래에 있을 수 있다. 결과로 단정하지 않고 사용자가 고르게 하면 **기본 동작을 나쁘게 만들지 않으면서** 이미 계산된 신호를 쓸 수 있다 — 타일 추론이 기각된 이유(기본 결과에 오검출을 섞는다)와 대비되는 지점이다. 고른 항목은 `FoodSource.Picked` 라 "직접 고름"으로 기록된다.
  - `CANDIDATE_FLOOR = 0.10`·`MAX_CANDIDATES = 8` 은 **아직 근거 없이 잡은 값**이다. `eval_heldout_colab.ipynb` 마지막 셀이 바닥값·개수를 훑어 "놓친 음식 중 몇 개를 후보가 담는지"를 낸다. 되찾는 비율이 낮으면 이 기능은 값어치가 없으니 뺀다.
