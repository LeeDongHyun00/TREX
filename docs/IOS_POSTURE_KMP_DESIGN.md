...
|---|---|---|
| iOS 지원 범위·Mac | 개발은 iOS 16+, Apple Silicon Mac·실제 iPhone을 가정. 장비/제품 요구는 미확인 | P0 |
...
당장 구현을 시작할 경우 첫 작업은 **P0의 기준 로그/테스트 확보와 P1의 계산 모듈 추출**이다. Swift 화면이나 C++ 변환부터 시작하지 않는다. 이 설계의 완료는 문서가 검토 가능한 상태라는 뜻이며, Android/iOS 구현·실기기 검증 완료를 뜻하지 않는다.
