...
const val ENGINE_VERSION = "trex-lab/0.1.0"
...
class LabEngine(val profile: ExerciseRepProfile, val pattern: RepMovementPattern = profile.defaultPattern) {
    ...
    fun process(timeMs: Long, features: Map<String, Float>, qualityOk: Boolean = true, floorSideConfirmed: Boolean = false): EngineOutput {
        ...
        if (last != null && timeMs-last > 1500) interrupt()
        lastTime = timeMs
        ...
        val observation = if (profile.isometric) {
    ...
    private fun output(t: Long, f: Map<String,Float>, e: List<ExerciseRepEvent>, status: String, view: String) = EngineOutput(
        ...
        FormMetrics.forExercise(profile).associate { it.key to f[it.key]?.takeIf(Float::isFinite) },
