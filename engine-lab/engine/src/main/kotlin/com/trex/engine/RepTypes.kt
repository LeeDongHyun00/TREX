...
internal class ReturnChannel(val signal: RepSignal, refractoryMs: Long, maxGapMs: Long) {
    private val fsm = ReturnRepTracker(signal.minAmp, refractoryMs, maxGapMs)
    var lastCycleMin = 0f; private set
    var lastCycleMax = 0f; private set
    fun reset() = fsm.reset()
    fun onObservationLost() = fsm.onObservationLost()
    fun onFrame(time: Long, value: Float?): Boolean {
