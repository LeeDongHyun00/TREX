...
class FloorFeatureExtractor {
    ...
    fun compute(normalizedXy: FloatArray, vis: FloatArray?, imageWidth: Int, imageHeight: Int): Map<String, Float> {
        ...
        if (vis != null) {
            ...
            if (core.any { !vis[it].isFinite() || vis[it] < 0.5f }) { reset(); return emptyMap() }
        }
        ...
        if (!vGround || !vWrist) { reset(); return f }
        if (histHip.size >= 600) { histHip.removeAt(0); histAnk.removeAt(0); histWr.removeAt(0) }
        // 접지선: 최근 유효 관측의 중앙값
